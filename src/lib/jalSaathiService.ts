...
export class JalSaathiService {
  ...
  async generateIrrigationSchedule(
    ...
    try {
      // Parse location and get weather forecast
      let weatherData;

      if (typeof location === 'string') {
        // Check if location string is formatted coordinates (e.g., "77.5610°N, 28.3475°E")
        const coordPattern = /(\d+\.?\d*)°?[NS],?\s*(\d+\.?\d*)°?[EW]/i;
        const match = location.match(coordPattern);

        if (match) {
          // Extract lat/lon from formatted string
          const lat = parseFloat(match[1]);
          const lon = parseFloat(match[2]);
          console.log(`✅ Parsed coordinates from formatted string: ${lat}, ${lon}`);

          try {
            weatherData = await weatherService.getWeatherByCoords(lat, lon);
          } catch (coordError) {
            console.warn('Failed to fetch weather by parsed coordinates, using Delhi as fallback');
            weatherData = await weatherService.getWeatherByCity('Delhi');
          }
        } else {
          // It's a city name
          try {
            ...
            console.warn(`Failed to fetch weather for ${location}, using Delhi as fallback`);
            weatherData = await weatherService.getWeatherByCity('Delhi');
          }
        }
      } else {
        ...
        try {
          weatherData = await weatherService.getWeatherByCoords(location.lat, location.lon);
        } catch (coordObjError) {
          console.warn('Failed to fetch weather by coordinates object, using Delhi as fallback');
          weatherData = await weatherService.getWeatherByCity('Delhi');
        }
      }
      ...
      if (!weatherData || !weatherData.daily || weatherData.daily.length === 0) {
        console.warn('Invalid weather data received, using Delhi as fallback');
        weatherData = await weatherService.getWeatherByCity('Delhi');
      }
  ...
  private calculateIrrigationSchedule(
    ...
    // Use daily forecast array (not forecast)
    const forecastDays = weatherData.daily || weatherData.forecast || [];
    ...
    // Process next 7 days
    for (let i = 0; i < 7; i++) {
      const forecastDay = forecastDays[i] || forecastDays[forecastDays.length - 1];
      const date = new Date();
  ...
  private generateIrrigationTips(
    ...
    const forecastDays = weatherData.daily || weatherData.forecast || [];
    const upcomingHeat = forecastDays.some((d: any) => d.temp_max > 35);
    const upcomingRain = forecastDays.some((d: any) => d.precipitation > 50);
  ...
  private generateAlerts(weatherData: any, schedule: IrrigationSchedule[]): string[] {
    ...
    // Use daily forecast array
    const forecastDays = weatherData.daily || weatherData.forecast || [];

    // Critical rain alerts with actions
    const heavyRainDays = forecastDays.filter((d: any) => d.precipitation > 70);
    const moderateRainDays = forecastDays.filter((d: any) => d.precipitation > 40 && d.precipitation <= 70);
    ...
    const extremeHeatDays = forecastDays.filter((d: any) => d.temp_max > 40);
    const hotDays = forecastDays.filter((d: any) => d.temp_max > 38 && d.temp_max <= 40);
    ...
    // Cold/frost alerts
    const coldDays = forecastDays.filter((d: any) => d.temp_min < 10);
    ...
    // Wind alerts
    const windyDays = forecastDays.filter((d: any) => d.wind_speed > 25);
    ...
    const goodDays = forecastDays.filter((d: any) =>
    ...
    // Disease risk alert
    const highHumidityDays = forecastDays.filter((d: any) => d.humidity > 80);
    ...
    // Pest risk alert
    const hotDryDays = forecastDays.filter((d: any) => d.temp_max > 32 && d.humidity < 50);
    ...
    const goodHarvestDays = forecastDays.filter((d: any) =>
      d.humidity < 60 &&
      d.precipitation < 10 &&
      d.wind_speed < 15
    );
    ...
    const goodFertilizerDays = forecastDays.filter((d: any) =>

      ...
      if (backendProxyUrl) {
        try {
          ...
          if (response.ok) {
            ...
            if (status === 503) {
              console.log('⚠️ Backend proxy temporarily unavailable (503)');
              console.log('   → Earth Engine service not initialized');
              console.log('   → Falling back to alternative data sources (NASA MODIS, POWER, etc.)');
            } else {
              console.log(`⚠️ Backend proxy responded with status ${status}`);
            }
          }
        } catch (proxyError: any) {
          const errorMsg = proxyError?.message || 'Unknown error';
          console.log('⚠️ Backend proxy not available:', errorMsg);
          if (errorMsg.includes('503')) {
            console.log('   → This is normal if Google Earth Engine credentials are not configured');
            console.log('   → App will use alternative data sources');
          }
        }
      } else {

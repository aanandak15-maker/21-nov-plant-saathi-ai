...
export class MarketIntelligenceService {
  ...
  async analyzeField(fieldId: string): Promise<FieldAnalysis> {
    ...
    if (fieldDataStr) {
      ...
      try {
        const field = await supabaseFieldService.getFieldById(fieldId);

        if (!field) {
          throw new Error(`Field data not found for ${fieldId}`);
        }

        const latestData = await supabaseFieldService.getLatestFieldData(fieldId);

        // Construct fieldData object compatible with analysis
        fieldData = {
          name: field.name,
          coordinates: field.coordinates,
          crop_type: field.crop_type,
          planting_date: field.created_at,
          region: field.location,
          latest_satellite_data: latestData ? {
            ndvi: latestData.ndvi,
            ndwi: latestData.ndwi,
            evi: latestData.evi,
            soil_moisture: latestData.soil_moisture,
            temperature: latestData.temperature,
            health_score: latestData.health_score,
            timestamp: latestData.timestamp
          } : {},
          marketplace_analysis: {
            // Default values for cold start
            npk_status: { nitrogen: 'medium', phosphorus: 'medium', potassium: 'medium' },
            growth_stage: { percentage: 50, stage_name: 'Vegetative', days_to_harvest: 60, confidence: 0.5 }
          }
        };

        // Cache for future use
        localStorage.setItem(fieldDataKey, JSON.stringify(fieldData));
      } catch (error) {
        console.error(`Failed to fetch field ${fieldId} from Supabase:`, error);
        throw new Error(`Field data not found for ${fieldId}`);
      }
    }

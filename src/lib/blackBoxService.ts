...
interface UserInteractionLog {
  timestamp: Date;
  interactionType: 'button_click' | 'audio_play' | 'scroll' | 'tab_switch' | 'error_retry' | 'page_view' | 'session_start' | 'user_identified' | 'field_creation';
  elementId: string;
  fieldId?: string;
...
interface ErrorLog {
  timestamp: Date;
  errorType: 'api_failure' | 'audio_failure' | 'component_error' | 'network_error' | 'storage_error';
  errorMessage: string;
  stackTrace?: string;
  fieldId?: string;
  userAction?: string;

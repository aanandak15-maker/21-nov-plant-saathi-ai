...
export const WeatherView = () => {
  ...
  const loadFields = async () => {
    try {
      ...
      // Map Supabase fields to local Field interface
      const mappedFields: Field[] = fieldsList.map((field: any) => {
        // Parse coordinates if they exist
        let location = undefined;
        if (field.coordinates && Array.isArray(field.coordinates) && field.coordinates.length > 0) {
          // Assuming coordinates are [[lng, lat], ...] or similar polygon structure
          // We'll take the first point as the location
          const firstPoint = field.coordinates[0];
          if (Array.isArray(firstPoint) && firstPoint.length >= 2) {
            location = { lat: firstPoint[1], lng: firstPoint[0] };
          }
        }
        ...
        return {
          ...
          sowingDate: field.lifecycle_metadata?.sowing_date,
          location: location,
          area: field.area
        };
      });
      ...
      if (mappedFields.length > 0 && !selectedFieldId) {
        const firstField = mappedFields[0];
                  ...
                  <div className="flex items-center justify-between">
                    <Badge variant="outline" className="text-sm">
                      {weatherData.daily.length === 5 ? (
                        ...
                        <>🌟 {weatherData.daily.length}-Day Extended Forecast</>
                      )}
                    </Badge>
                    {weatherData.daily.length === 5 && (

...
import { AlertTriangle, Droplets, Thermometer, Bug, TrendingUp, CheckCircle2, Volume2, ArrowRight } from "lucide-react";
...
export const NotificationsView = () => {
  ...
  const markAllAsRead = () => {
    const allIds = generateAllAlerts().map(a => a.id);
  ...
  const speakNotification = (text: string) => {
    if ('speechSynthesis' in window) {
      ...
      // Try to find a Hindi voice if available, otherwise fallback to default
  ...
  const generateAllAlerts = () => {
    ...
    for (const field of fieldsData) {
      ...
      if (field.ndvi && field.ndvi < 0.6) {
        ...
        alerts.push({
          id: `ndvi-${field.id}-${new Date().getDate()}`,
          type: "critical",
          ...
          onAction: () => navigate(`/fields/${field.id}`)
        });
      }
      ...
      if (field.ndwi && field.ndwi < 0.3 && field.moisture && field.moisture < 30) {
        alerts.push({
          id: `water-${field.id}-${new Date().getDate()}`,
          type: "critical",
          category: "field_health",
          icon: Droplets,
          ...
          color: "border-red-500 bg-red-50",
          timestamp: new Date().toISOString(),
          actionLabel: "View Irrigation Plan",
          onAction: () => navigate(`/fields/${field.id}`)
        });
      }
    }
    ...
    if (currentHour >= 5 && currentHour < 10 && irrigationData) {
      ...
      if (needsIrrigation) {
        alerts.push({
          id: `irrigation-window-${new Date().getDate()}`,
          type: "warning",
          category: "irrigation",
          icon: Droplets,
          ...
          onAction: () => markAsRead(`irrigation-window-${new Date().getDate()}`)
        });
      }
    }
    ...
    if (currentHour >= 5 && currentHour < 10 && weatherData) {
      ...
      if (windSpeed < 10) {
        alerts.push({
          id: `spray-window-${new Date().getDate()}`,
          type: "info",
          category: "spray",
          icon: AlertTriangle,
    ...
    if (weatherData) {
      ...
      if (current.temp > 40) {
        alerts.push({
          id: `heat-alert-${new Date().getDate()}`,
          type: "critical",
          category: "weather",
          icon: Thermometer,
      ...
      const heavyRain = daily?.find((d: any) => d.precipitation > 70);
      if (heavyRain) {
        alerts.push({
          id: `rain-alert-${new Date().getDate()}`,
          type: "warning",
          category: "weather",
          icon: Droplets,
      ...
      if (current.temp < 10) {
        alerts.push({
          id: `frost-alert-${new Date().getDate()}`,
          type: "critical",
          category: "weather",
          icon: AlertTriangle,
          title: "❄️ FROST RISK",
      ...
      if (current.humidity > 85) {
        alerts.push({
          id: `disease-risk-${new Date().getDate()}`,
          type: "warning",
          category: "disease",
          icon: Bug,
          ...
          color: "border-orange-500 bg-orange-50",
          timestamp: new Date().toISOString(),
          actionLabel: "Check Disease",
          onAction: () => navigate('/disease')
        });
      }
    }
    ...
    if (irrigationData?.alerts && irrigationData.alerts.length > 0) {
      irrigationData.alerts.forEach((alert: string, index: number) => {
        if (alert.includes("ALERT") || alert.includes("URGENT")) {
          alerts.push({
            id: `irrigation-alert-${index}-${new Date().getDate()}`,
            type: "info",
            category: "irrigation",
            icon: Droplets,
    ...
    if (diseaseOutbreaks.length > 0) {
      ...
      recentOutbreaks.forEach((outbreak, index) => {
        alerts.push({
          id: `disease-outbreak-${index}-${new Date().getDate()}`,
          type: "critical",
          category: "disease",
          icon: Bug,
    ...
    if (irrigationData?.weeklyWaterSavings > 20) {
      alerts.push({
        ...
        timestamp: new Date().toISOString(),
        actionLabel: "Awesome!",
        onAction: () => markAsRead(`savings-${new Date().getDate()}`)
      });
    }
  ...
  const allAlerts = generateAllAlerts().filter(a => !readNotifications.includes(a.id));
              ...
              <AlertDescription className="w-full">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => speakNotification(alert.message)}
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  {alert.actionLabel && (
                    <Button
                      size="sm"
                      className="h-7 text-xs bg-white/90 text-black hover:bg-white border shadow-sm"

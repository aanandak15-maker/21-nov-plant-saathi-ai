...
import { ArrowLeft, Volume2, Camera, TrendingUp, MapPin, Edit, Archive } from "lucide-react";
...
export const FieldDetailsDashboard = () => {
        ...
        try {
          ...
          if (storedField) {
            ...
            parsedField = {
              ...
              expectedHarvestDate: parsedField.expectedHarvestDate,
              coordinates: parsedField.coordinates || [],
              irrigationMethod: parsedField.irrigationMethod || parsedField.irrigation_method || "Not specified",
              wateringFrequency: parsedField.wateringFrequency
            };
          }
  ...
  const handleUpdateField = async (updates: any) => {
    ...
    const fieldUpdates = {
      ...
      sowing_date: updates.sowingDate,
      lifecycle_metadata: {
    ...
    const updated = await supabaseFieldService.updateField(fieldId, fieldUpdates);
  ...
  const handleArchiveField = async () => {
    ...
    if (confirm("Are you sure you want to mark this field as harvested? It will be archived.")) {
      const { supabaseFieldService } = await import('@/lib/supabaseFieldService');
      await supabaseFieldService.updateField(fieldId, { status: 'harvested' });
      toast({ title: "Field Archived", description: "Field marked as harvested." });
      navigate('/soilsaathi');
    }
  };
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="flex gap-2">
            ...
            <Button
              variant="outline"
              size="sm"
              className="bg-white/10 text-white border-white/20 hover:bg-white/20 hover:text-red-300 hover:border-red-300"
              onClick={handleArchiveField}
            >

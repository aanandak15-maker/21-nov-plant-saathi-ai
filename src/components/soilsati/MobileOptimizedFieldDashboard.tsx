...
// Extended Field interface to include frontend-specific properties
...
export const MobileOptimizedFieldDashboard = () => {
  ...
  const [field, setField] = useState<ExtendedField | null>(null);
        ...
        if (storedField) {
          ...
          if (foundField) {
            const formattedField = {
              ...
              cropType: foundField.crop_type,
              variety: foundField.variety || "Standard",
              area: foundField.area || 0,
              sowingDate: foundField.sowing_date,
              expectedHarvestDate: foundField.expectedHarvestDate || new Date(new Date(foundField.sowing_date || Date.now()).getTime() + 120 * 24 * 60 * 60 * 1000).toISOString(),
              coordinates: foundField.coordinates || [],
              irrigationMethod: foundField.irrigationMethod || "Not specified",
              health: (foundField as any).health || { ndvi: 0, status: "unknown" }
            };
            setField(formattedField);
          } else {
            toast({
              title: "Error",
  ...
  useEffect(() => {
    ...
    const polygonPoints = field.coordinates.map((c: any) => [c[0], c[1]] as [number, number]);
